    ...
    def __init__(self, N_input, N_hidden, N_output):
        ...
        if len(N_hidden) > 1:
            for i in range(1, len(N_hidden)):
                self.node_network.add_module(f"l{i}", nn.Linear(N_hidden[i - 1], N_hidden[i])),
                self.node_network.add_module(f"r{i}", nn.ReLU())


        self.node_network.add_module("l-1", nn.Linear(N_hidden[-1], N_output))
...
class MoleculeMPNN(nn.Module):
    ...
    def forward(self, g):        
        ...
        g.nodes["cell"].data['h'] = self.node_encoding(features.float())
